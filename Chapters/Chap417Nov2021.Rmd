...
an expression that introduces the common actuarial notation for the expected present value of this contract. Figure \@ref(fig:MBaseLifeInsPlot) summarizes the expected presented values by age $x$ over choices of $m=1,2,4, 12$. As is evident from the plot, these values are close and in fact ordered as
...
To simplify the presentation, we again assume a constant spot rate $i$ so that $v^t =(1+i)^{-t}$. 
...
\end{array}
$$
where $d^{(m)} = m\left[1- v^{1/m}\right]$ is an $m^{th}$ly discount factor and $\beta(m) = \frac{i-i^{(m)}}{i^{(m)}d^{(m)}}$ is another factor.
...
v^{T_{\bf x}}I(T_{\bf x} +v^n I(T_{\bf x} >n) & {n\text{ year  endowment}}.\\
\end{array}
\right.
$$
...
\end{array}
$$
When a continuous time probability model is available, then these integrals can be computed directly (although sometimes require numerical integration software). An alternative approach is to the approximation based on $m^{th}$ly payments introduced in the prior section and let $m \to \infty$. For example, for the whole life assuming *UDD*, we have
