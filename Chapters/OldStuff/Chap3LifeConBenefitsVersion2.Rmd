...
&= \sum_{k=0}^{\infty} ~_kC~ v^k ~\mathrm{E}(_kB) \\
...
The calculation in equation \@ref(eq:Benefit3) does not depend on the dependence relationships among the $_kB$ random variables as expectations of random variables are not affected by correlations between random variables.
...
**Example 3.1. Ten Year Cash Flows.** To illustrate, we consider ten years of cash flows at times $k \in {0,1,2,...,10}$. Cash flow amounts $_kC$, one period spot rates $_k i$, and probabilities of occurrence probability ($\Pr(_kC) = \Pr( _kB=1)$) are given in the following [Display 3.1]. These inputs are highlighted (in yellow). This display shows the calculation of $\mathrm{E}(PV)$ and $\mathrm{Var}(PV)$ for a cash flow model of the style shown in equations \@ref(eq:Benefit2), \@ref(eq:Benefit3) and \@ref(eq:Benefit4), assuming that only one cash flow is possible. 
...
Note also that if we assume $_ki=i$ (i.e. that that yield curve is flat), then the $PV$ in equations \@ref(eq:Benefit7) and \@ref(eq:Benefit8) above can also be expressed using actuarial notation from financial mathematics. For example, for a life annuity in advance:
