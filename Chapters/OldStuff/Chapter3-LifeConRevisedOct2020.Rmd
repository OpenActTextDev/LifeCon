...
<div id="containerCPE"></div>

```{r echo=ECHOCODE}
# Import data from CSV file, then convert so it can be used in Javascript
dataCPE <- read.csv("ContingentPaymentExample.csv", header = F)
...
var updateCPE = function (instance, cell, x, y, value) {
  z = mySpreadsheetCPE.tbody.rows.item(y).cells; var plotseries = myPlotSeries(z); 
// The first series to be plotted is in row 9.   
if (y >= 8 && y < 12 ) {
      if (x == 0) {
            // Update legend
            chartCPE.series[y-8].update({ name:value });
            } else if (x < plotseries.length+1) {
            // Update chart data
            chartCPE.series[y-8].data[x-1].update({ y:plotseries[x-1] });
            }
        }     
    };
...
```{js echo=ECHOCODE}
dataCPE[7]["V14"]="Sum"; // AB - I'm not sure what this line of code does
// Spreadsheet Defined
