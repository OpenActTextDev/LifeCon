...
We have now seen the foundations of life contingencies; Chapter \@ref(C:ModelingLifeTimes) on survival modeling and Chapter \@ref(C:SimpleBenefit) on discounting of contingent cash flows. This chapter provide extensions and variations to exhibit the richness of applications.

Specifically, Section \@ref(Sec:mthlyFlows) presents a relatively minor extension theoretically, allowing for $m^{th}$ly cash flows, that has extensive practical impact for retirement benefits, other life contingent annuities, and premium payment plans. Section \@ref(Sec:SamplingBias) sharpens our intuition about survival modeling by linking the choice of a survival dataset to an application under consideration. Section \@ref(Sec:EconomicMotivation) provides economic motivation for some of the measures summarizing insurance features introduced in Chapter \@ref(C:SimpleBenefit); this motivation provides insights as why some of these are so important in practice. Through a variety of examples, Section \@ref(Sec:ApplicationsContingent) shows that the framework for life contingent benefits applies to contingent payment problems that go beyond applications in only the life insurance industry.
...
When describing benefits paid at discretely, we started in Chapter \@ref(C:SimpleBenefit) with annual payments because mortality tables have traditionally been presented on an integer age basis. In addition, business requirements such as insurance company financial reporting is done routinely on an annual basis. Nonetheless, as described above, there are also important situations when we need to handle situations where payments occur than annually (typically more frequently, although payments once every, e.g., two years are certainly plausible).
...
$$
\begin{array}{l}
PV = \sum_{k=0}^{\infty} v^{k/m} ~_{k/m}C~_{k/m}B~   .\\
\end{array}
(\#eq:Benefitm2)
$$

For probabilities associated with the cash flow indicator, as we saw in Chapter 3, we are concerned with payments either at the death or the survival of a policyholder age $x$. That is,

$$
\mathrm{E} \left(_{k/m}B\right) = \left\{\begin{array}{ccc}
\Pr\left(\frac{k-1}{m} < T_x \le \frac{k}{m}\right) & = & ~_{(k-1)/m} p_x - ~_{k/m} p_x& {\small \text{ prob of death}} \\
\Pr\left(T_x \ge \frac{k}{m}\right) & =& ~_{k/m} p_x & {\small \text{ prob of survival} }.\\
\end{array}
\right.
(\#eq:ExpBenefitm2)
$$
...
**Uniform Distribution of Deaths.** Only in the case where we have annual mortality with $m^{th}$ly cash flows are some extensions required. And we already started on this in Section \@ref(S:LifeTabFrac) where fractional year assumptions were introduced. To see how this works with $m^{th}$ly cash flows, we use the uniform distribution of death (*UDD*) assumption and recall that we can express $~_t p_x = 1 - t \times q_x$, for $0 \le t \le 1$. So, if $j$ is a counter for the number of $m^{th}$ly units, we have
...
Let us see how these ideas work with a few illustrative examples. To begin, we start by assuming that we have a continuous time probability model and so no approximations nor assumptions are needed.

<a id=tab:Ex4.1></a> 

[Example 4.1]: ../docs/C-BenefitExt.html##tab:Ex4.1

**Example 4.1. Whole Life Insurance with $m^{th}$ly Payments with Continuous Time Mortality**. We return to [Example 3.2] that used the Section \@ref(S:SurvReg) mortality. Now only consider the baseline case in [Example 3.2] that you will recall is  a healthy female who is not a smoker, not obese, and has an average blood pressure level equal to 114, as well as an interest rate of 4%.

To calculate the expected value of the policy, we may use equations \@ref(eq:Benefitm2) and \@ref(eq:ExpBenefitm2) directly with the potential cash flows identically equal to one $~_{k/m}C \equiv 1$. The resulting expected present value is
...
Figure \@ref(fig:MBaseLifeInsPlot) summarizes the expected presented values $A_x$ by age $x$ over choices of $m=1,2,4, 12$. As is evident from the plot, these values are close and in fact ordered as

$$
A_x = A_x^{(1)} \le A_x^{(2)} \le A_x^{(4)} \le A_x^{(12)} ~.
$$

The annual expected presented value, $A_x = A_x^{(1)}$, is the smallest because it is the contract with payments potentially farthest in the future. For example, if a person dies just before one month into a contract year, then the annual payment is 11 months after death. In contrast, it is five months after death when $m=2$, two months later for $m=4$ and virtually instantaneous for $m=12$. So, the very largest the separation that could be in Figure \@ref(fig:MBaseLifeInsPlot) is $1/(1+i) = 1/1.04$, for this example.
...
**Example 4.2. Whole Life Insurance with $m^{th}$ly Payments with Annual Mortality**. Let us now focus on additional adjustments needed when we have only traditional life table mortality at integer (or annual) ages. 

To calculate the expected value of the policy, we follow the procedure of splitting up the payment stream into yearly intervals and then applying the *UDD* assumption within each year.
...
To simplify the presentation, we now assume a constant spot rate $i$ so that $v^t =(1+i)^{-t}$. With this, we an employ standard geometric sums that many readers will recall from interest theory,
...
**Example 4.3. Term Life Annuity with $m^{th}$ly Payments**

For a more complex example, we now consider the case of an $n$ year term life annuity with payments in advance. We also use this special case to introduce some notation commonly used in actuarial practice as well as some approximations that are useful for interpretations. By convention, when presenting actuarial symbols, each payment (when the person is alive) is assumed to be $1/m$, not 1. This is so that we can readily compare, for example, annuities based on quarterly to monthly payments because they both pay a sum of 1 over the year if the policyholder survives.
...
\begin{array}{rl}
 ...
 &= \frac{1}{m} \sum_{r=0}^{nm-1} v^{r/m} ~_{r/m}p_{\bf{x}} \\
 &= \frac{1}{m} \sum_{k=0}^{n-1} \sum_{j=0}^{m-1} v^{k+j/m} ~_{k+j/m}p_{\bf{x}} \\
...
$$
\begin{array}{cccccc}
\frac{1}{m}\sum_{j=0}^{m-1} v^{j/m} &=& \frac{1}{m}\frac{1-v}{1-v^{1/m}} &= \ddot{a}_{\enclose{actuarial}{1}}^{(m)} & \approx 1 \\
\frac{1}{m}\sum_{j=0}^{m-1} j~v^{j/m} &=& \frac{v^{1/m}}{m} \frac{1-v}{(1-v^{1/m})^2} - \frac{v}{1-v^{1/m}} &= (I^{(m)}\ddot{a})_{\enclose{actuarial}{1}}^{(m)} & \approx \frac{m-1}{2} .
\end{array}
$$
...
\begin{array}{rl}
\ddot{a}_{{\bf{x}}:\enclose{actuarial}{n}}^{(m)} 
 &=_{UDD} \ddot{a}_{\enclose{actuarial}{1}}^{(m)}  \sum_{k=0}^{n-1}  v^{k} ~_k p_x -
 (I^{(m)}\ddot{a})_{\enclose{actuarial}{1}}^{(m)} \frac{1}{m} \sum_{k=0}^{n-1}  ~_k p_x q_{x+k} \\
 &= \ddot{a}_{\enclose{actuarial}{1}}^{(m)}  \ddot{a}_{{\bf{x}}:\enclose{actuarial}{n}} -
 (I^{(m)}\ddot{a})_{\enclose{actuarial}{1}}^{(m)} \frac{1}{m} A_{{\bf{x}}:\enclose{actuarial}{n}} ^ {1} \\
