...
xage <- seq(30,120)
q.x.function <- function(time=65+5,x=xage, male=0, bpress=114.06, smoke=0, obese=0){
  c.x <- cons*exp( coeff_male * male + coeff_bpress * bpress + 
         coeff_smok * smoke + coeff_obese * obese)
  arg <- -c.x*((exp(coeff_age)-1)/coeff_age)*exp(time_fac)*exp(coeff_age*xage)
  q.x <- 1-exp( arg)  
  return(q.x)
}
...
q.mat <- matrix(0, length(xage),length(bpress.vec) )
for (j in 1:length(bpress.vec)){q.mat[,j] <- q.x.function(bpress=bpress.vec[j]) }
matplot(xage,q.mat, xlab="Age x", ylab = expression(A[x]), type = "l", col=rgb(0,0,1,1,0.2), xlim=c(60,90), ylim=c(0,0.60),
...
ggplot(dataf,aes(x=xage, y=A.x.base)) + theme_bw() +
  ...
  annotate("text", x=40, y=0.0016, size=3, label = "n = 3")+
  annotate("text", x=40, y=0.013, size=3, label = "n = 10") + 
  annotate("text", x=40, y=0.100, size=3, label = "n = 30")
...
To see how valuation of these uncertain cash flows work, we start by examining a single policy from the portfolio. Specifically, we consider a female aged $x=50$ at the purchase of the policy. Further we assume this individual has purchased a term insurance policy with term $n=10$ years with a sum insured of \$400,000 ($=~_kC$) and that we wish to value the policy immediately after purchase. 

As noted above, the mortality probability is expected to follow that described in Exercise 3.2. In these calculations, we start by assuming an interest rate of 2% per annum effective. 
...
$$
PV_j = v^{K_{{\bf{x}}_j}+1} I({K_{{\bf{x}}_j}+1}+1 \le n_j)  .
$$
