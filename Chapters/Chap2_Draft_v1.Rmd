---
title:  "The Mathematics, Statistics, and Economics of Life Insurance"
...
library(knitr)
us_les <- read.csv("SuppMaterial/CDCLifeExp.csv")
...
```{r, echo = TRUE}
us_exp <- read.csv("SuppMaterial/HMD_Expo.csv")
kable(head(us_exp), align = "cccrrr", digits = 2, format.args = list(big.mark = ","))
```
and deaths
```{r, echo = TRUE}
us_deaths <- read.csv("SuppMaterial/HMD_Deaths.csv")
...
```{r, echo = TRUE}
library(data.table)
ins_data <- fread("SuppMaterial/SyntheticInsurerData.csv",data.table=FALSE)
...
tmp_describe_ins_data<-describe(ins_data)
...
```{r P0USFemMalDataFigs, comment="", message = FALSE, warning = FALSE, fig.align="center",  fig.width=8, fig.height=4, fig.cap = 'One year death probability curve for newborn Femals (left) and Males (right) for the 2010-2014 U.S. data across all ages'}
par(mfrow=c(1,2))
